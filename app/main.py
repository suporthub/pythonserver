...
# --- Barclays Margin Enforcer Task ---
async def run_barclays_margin_enforcer():
    """
    Background task to enforce margin requirements for Barclays users by cancelling their highest-margin pending orders
    until the user's free margin is sufficient to cover all remaining pending orders.
    """
    logger = logging.getLogger("barclays_margin_enforcer")
    logger.setLevel(logging.INFO)
    log_path = os.path.join(os.path.dirname(__file__), '..', 'logs', 'barclays_margin_enforcer.log')
    ...
    logger.info("Starting Barclays margin enforcer background task.")

    from app.core.cache import get_user_dynamic_portfolio_cache, get_group_settings_cache
    from app.crud import crud_order, user as crud_user
    from app.api.v1.endpoints.orders import get_order_model, cancel_pending_order
        ...
        try:
            async with AsyncSessionLocal() as db:
                if not global_redis_client_instance:
                    ...
                    continue
                # Get all active users (live only)
                ...
                for user in live_users:
                    ...
                    if not group_settings:
                        continue
                    sending_orders = group_settings.get('sending_orders', '').lower()
                    if sending_orders != 'barclays':
                        continue
                    # Fetch free margin from dynamic portfolio cache
                    dynamic_portfolio = await get_user_dynamic_portfolio_cache(global_redis_client_instance, user_id)
                    if not dynamic_portfolio:
                        continue
                    try:
                        free_margin = float(dynamic_portfolio.get('free_margin', 0))
                    except Exception:
                        free_margin = 0
                    ...
                    pending_orders = await crud_order.get_orders_by_user_id_and_statuses(db, user_id, pending_statuses, order_model)
                    if not pending_orders:
                        continue
                    # Sum required margin for all pending orders
                    pending_orders_with_margin = [o for o in pending_orders if getattr(o, 'margin', None) is not None]
                    total_required_margin = sum(float(o.margin) for o in pending_orders_with_margin)
                    # If free margin is enough, nothing to do
                    if free_margin >= total_required_margin:
                        continue
                    # Sort pending orders by margin descending (highest first)
                    sorted_orders = sorted(pending_orders_with_margin, key=lambda o: float(o.margin), reverse=True)
                    # Cancel highest-margin orders one by one until free margin is enough
                    for order in sorted_orders:
                        # Re-fetch dynamic portfolio and recalculate total required margin
                        dynamic_portfolio = await get_user_dynamic_portfolio_cache(global_redis_client_instance, user_id)
                        try:
                            free_margin = float(dynamic_portfolio.get('free_margin', 0))
                        except Exception:
                        ...
                        if free_margin >= total_required_margin:
                            break
                        # Cancel this order (use the same logic as cancel_pending_order endpoint)
                        from app.schemas.order import PendingOrderCancelRequest
                        cancel_req = PendingOrderCancelRequest(
                            order_id=order.order_id,
                            user_id=user_id,
                            user_type=user_type,
                            symbol=order.order_company_name,
                            ...
                            cancel_message="Auto-cancelled due to insufficient free margin"
                        )
                        try:
                            # Use the same DB and Redis client
                            ...
                            logger.info(f"Auto-cancelled pending order {order.order_id} for user {user_id} (margin={order.margin})")
                        except Exception as e:
                            logger.error(f"Failed to auto-cancel order {order.order_id} for user {user_id}: {e}")
                        await asyncio.sleep(0.5)  # Small delay to avoid hammering
        except Exception as e:
            logger.error(f"Error in Barclays margin enforcer: {e}")
...
async def startup_event():
    ...
    try:
        ...
        if redis_available and global_redis_client_instance:
            ...
            # Add Barclays margin enforcer
            barclays_margin_task = asyncio.create_task(run_barclays_margin_enforcer())
            background_tasks.add(barclays_margin_task)
            barclays_margin_task.add_done_callback(background_tasks.discard)
